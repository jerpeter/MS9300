...
///	Local Scope Globals
///----------------------------------------------------------------------------
// CRC8 implementation according to CCITT standards
...
static const uint16 s_CRCCCITT16TABLE[TABLE_SIZE] =
{
...
// CRC16 implementation according to X16 + X15 + X2 + 1 polynomial
static const uint16 s_CRC16TABLE[TABLE_SIZE] =
{
	...
	0xD581, 0x1400, 0xD4C1, 0x1540, 0xD701, 0x17C0, 0x1680, 0xD641,
...
// CRC32 implementation according to CCITT standards
static const uint32 s_CRCCCITT32TABLE[TABLE_SIZE] =
{
...
///	Function Break
///----------------------------------------------------------------------------
uint16 CalcCrc16(uint8* data, uint32 length, uint16 seed)
{
	uint32 i;

	for (i = 0; i < length; i++)
	{
		seed = (seed << 8) ^ s_CRC16TABLE[(seed >> 8) ^ data[i]];
	}
...
///	Function Break
///----------------------------------------------------------------------------
uint16 CalcCCITT16(uint8* data, uint32 length, uint16 seed)
{
	uint32 i;

	for (i = 0; i < length; i++)
	{
		seed = (seed << 8) ^ s_CRCCCITT16TABLE[(((seed >> 8) ^ data[i]) & 0xFF)];
	}
